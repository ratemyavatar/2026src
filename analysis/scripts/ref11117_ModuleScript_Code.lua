	...
	function Expand(ent, point, text, richText)
		...
		if label then
			...
			ent.MouseMoved:Connect(function(x,y)
				local sizeText = text or ent.Desc.Value
				...
				if richText then
					sizeText = string.gsub(sizeText, '\<[^>]+\>', '');
					label.TextScaled = true;
				end

				--point.Size = UDim2.new(0, 10000, 0, 10000)
				local newx = 500
				local bounds = service.TextService:GetTextSize(sizeText, label.TextSize, label.Font, Vector2.new(1000,1000)).X-- point.Label.TextBounds.X
