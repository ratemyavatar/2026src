...
RANK_NAME[RANK_HEADADMIN] = "Head Admin"
...
ADONIS_RANK[RANK_HEADADMIN] = "HeadAdmins"
...
local function SetStaffRank(actor, targetUserId, rank, displayName)
	...
	if rank >= RANK_HEADADMIN and myRank < RANK_OWNER then
		return nil, "Only an Owner can hand out Head Admin and above."
	end
...
AddCommand("headadmin", {
	Rank = RANK_OWNER,
	Args = {"player"},
	Label = "Make Head Admin",
	Blurb = "Give them the Head Admin rank. Owner only.",
	Run = function(actor, target)
		return SetStaffRank(actor, target.UserId, RANK_HEADADMIN, target.Name)
	end,
})
...
-- Custom chat tags, keyed by UserId. These override the rank tag in chat.
-- Thugshaker keeps "[Thug]" in yellow-gold instead of his Owner tag.
local CUSTOM_CHAT_TAGS = {
